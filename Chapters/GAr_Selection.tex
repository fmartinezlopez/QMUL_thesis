...
Now that I have checked the robustness of the proposed $\nu_{\mu}$ CC selection, it can be used as a starting point for other, more convoluted, selections. One of the priorities of ND-GAr, as mentioned previously, is the identification of pions. In a 

The $\nu_{\mu}$ CC selection provides a starting point for the pion identification. The first thing one can do is rule out the primary muon candidate. Then, the rest of the reconstructed particles

proton scores can be used to the protons from the sample of charged pions. providing cuts for these a separation can be achieved

the $\mathrm{d}E/\mathrm{d}x$ of the track associated to the reconstructed particle. To select the charged pions, we can require that the is compatible with the expectation for a true $\pi^{\pm}$, 
\begin{eqnarray}
    \left<\frac{\mathrm{d}E}{\mathrm{d}x}\right>_{\pi^{\pm}} \left(1 - \Delta_{\mathrm{d}E/\mathrm{d}x}^{\pi^{\pm}}\right) \leq \left<\frac{\mathrm{d}E}{\mathrm{d}x}\right>_{meas.} < \left<\frac{\mathrm{d}E}{\mathrm{d}x}\right>_{\pi^{\pm}} \left(1 + \Delta_{\mathrm{d}E/\mathrm{d}x}^{\pi^{\pm}}\right).
\end{eqnarray}
To obtain the expected mean $\mathrm{d}E/\mathrm{d}x$ of a charged pion of a given momentum, I use the ALEPH parametrisation with the parameter values obtained previously.

interested in primary muons, these are associated to the primary vertex of the interaction. use position of the muon candidate to restrict the starting point of pions to a certain volume around it

\begin{figure}[t]
    \centering
    \includegraphics[width=.90\linewidth]{Images/GAr_selection/pion_selection_0_pions_metrics.pdf}
...
I propose the following procedure identifying charged pions in the sample
\begin{enumerate}
    \item Apply $\nu_{\mu}$ CC selection.
    \item Disregard particle selected as primary muon.
    \item Remove particles with momentum below threshold.
    \item Select particles with proton dE/dx score above threshold.
    \item Select particles with proton ToF score above threshold
    \item Select particles with mean dE/dx around the expectation for a pion.
    \item Measure distance between the beginning of the track and the primary muon.
\end{enumerate}

\begin{figure}[t]
    \centering
    \includegraphics[width=.90\linewidth]{Images/GAr_selection/pion_selection_1_pions_metrics.pdf}
