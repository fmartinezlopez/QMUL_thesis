...
introducing an arbitrary number of sterile neutrinos, $N_{i}$, $i=1,\dots,m$.
\begin{equation}
	-\mathcal{L}_{M_{\nu}} = M^{ij}_{D} \bar{N}_{i} \nu_{Lj} + \frac{1}{2} M^{ij}_{N} \bar{N}_{i} N^{c}_{j} + \mathrm{h.c.},
\end{equation}
where $M_{D}$ is a complex $m \times 3$ matrix and $M_{N}$ a complex, symmetric $m \times m$ matrix. The first term, often referred to as the Dirac mass term, arises from the corresponding Yukawa interaction after the spontaneous electroweak symmetry breaking, the other fermions. The second term, Majorana mass term, is allowed in the Lagrangian,singlet of the gauge group. However, it violates lepton number conservation by two units.
\begin{equation}
