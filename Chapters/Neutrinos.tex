...
In the case of neutrino interactions with nuclei at the energies relevant for long baseline oscillation experiments, cross section for the neutrino-nucleon interaction. Figure \ref{fig:neutrino_cc_interactions} shows examples of the four most common neutrino CC interactions.
...
Nuclear effects alter the neutrino cross section, as well as the final states particles. There are several models available to describe the initial state of the nucleus, like the relativistic Fermi gas model, spectral functions or the random phase approximation. Figure \ref{fig:fsi_diagram} illustrates the effects of FSI on the observable particle content in the detector after a $\nu_{\mu}$ CCQE interaction.
