...
\begin{comment}
\gls{ndgar} is a magnetised, high-pressure gaseous argon \gls{tpc} (\gls{hpgtpc}), surrounded by an electromagnetic calorimeter (\gls{ecal}) and a muon detector (commonly refer to as \gls{muid}). A detailed discussion on the requirements, design, performance and physics of \gls{ndgar} can be found in the \gls{dune} ND \gls{cdr} \cite{DUNE2021NDCDR} and the \gls{ndgar} whitepaper (cite).

In \gls{dune} Phase II \gls{ndgar} will fulfill the role of TMS, measuring the momentum and sign of the charged particles exiting \gls{ndlar}. Additionally, it will be able to measure neutrino interactions inside the \gls{hpgtpc}, achieving lower energy thresholds than those of the ND and \gls{fd} \gls{lartpc}s. By doing so \gls{ndgar} will allow to constrain the relevant systematic uncertainties for the \gls{lbl} analysis even further.

The goal of the present chapter is to review the requirements that the physics program of \gls{dune} impose on \gls{ndgar}, present the current status of its design and describe the GArSoft package, its simulation and reconstruction software.

As decided during the \gls{dune} Phase II workshop in June 2023 [reference], we want to build \gls{ndgar} physics case by showing:
\begin{itemize}
    \item That \gls{ndgar} can constrain systematic uncertainties that \gls{ndlar} might miss.
    \item The impact on the neutrino oscillation results if such systematic uncertainties are missed.
    \item That \gls{ndgar} is necessary to reach \gls{dune}'s main physics goals.
\end{itemize}

This way, the design of \gls{ndgar} will be physics driven.

In order to study the effects of final state interactions (\gls{fsi}) in \gls{cc} interactions, \gls{ndgar} should be able to measure the spectrum of protons and charged pions at low energies. \gls{ndgar} also needs to be able to measure the pion multiplicity, specially for energies above $100~\mathrm{MeV}$ as at these energies the pions shower in the \gls{lar}, to inform the pion mass correction in the ND and \gls{fd} \gls{lartpc}s.

In order to correctly identify electrons, muons, pions, kaons and protons \gls{ndgar} can use a combination of: $\mathrm{d}E/\mathrm{d}x$ measurements in the \gls{hpgtpc}, $E_{ECAL}/p$ using the ECAL total energy and the momentum obtained from magnetic spectroscopy in the \gls{hpgtpc} and penetration information through the ECAL and muon tagger.
\end{comment}

% more on ND-GAr and the ND, and why we want to do this!
In \gls{dune} Phase II, \gls{ndgar} will fulfill the role of TMS measuring the momentum and sign of the charged particles exiting \gls{ndlar}. Additionally, it will measure neutrino interactions inside the \gls{hpgtpc}. This way, \gls{ndgar} will allow to constrain certain cross-section systematic uncertainties and study the effect of \gls{fsi} in \gls{cc} interactions. To do so, it needs to measure the spectrum of protons and charged pions at low energies, as well as the pion multiplicity. This puts strong requirements to the particle identification (\gls{pid}) capabilities of the detector, as well as stimulates the relevant developments in the reconstruction.
...
The current \gls{tpc} track-\gls{ecal} cluster association algorithm is divided in four parts. It first checks whether the track end point fulfils certain conditions to be extrapolated. There are two cut values in this step, one for the drift direction and other radial.
...
One of the possible weak points of this approach is that it relies on the position along the drift direction to make the decisions. Within the current \gls{ndgar} design implemented in GArSoft, the timing information is provided by the \gls{ecal}. That effectively means that prior to make the track-\gls{ecal} associations the reconstructed $x$ positions of the track trajectories differ from the simulated ones by an amount:
\begin{equation}
